...
class ContactBook:
    ...
    def view_contacts(self):
        if not self.contacts:
            ...
            self.show_large_message("Contact List", contact_list)
    ...
    def search_contact(self):
        ...
        if search_term:
            ...
            if results:
                ...
                self.show_large_message("Search Results", contact_list)
            else:
        ...
        if search_term:
            for contact in self.contacts:
                if search_term.lower() in contact['Name'].lower() or search_term in contact['Phone']:
                    updated_name = simpledialog.askstring("Update Contact", f"Update Name (Current: {contact['Name']}):", initialvalue=contact['Name'])
                    updated_phone = simpledialog.askstring("Update Contact", f"Update Phone (Current: {contact['Phone']}):", initialvalue=contact['Phone'])
                    updated_email = simpledialog.askstring("Update Contact", f"Update Email (Current: {contact.get('Email', '')}):", initialvalue=contact.get('Email', ''))
                    updated_address = simpledialog.askstring("Update Contact", f"Update Address (Current: {contact.get('Address', '')}):", initialvalue=contact.get('Address', ''))
    ...
    def show_large_message(self, title, content):
        """ Display large contact lists in a new window instead of messagebox. """
        top = tk.Toplevel(self.window)
        top.title(title)
        text = tk.Text(top, wrap="word", width=50, height=15)
